...
std::string TodoStorage::getStoragePath() {
    ...
    // Linux: ~/.local/share/[AppName]/todos.json
    ...
    // Проверяем наличие XDG_DATA_HOME
    const char* xdgDataHome = getenv("XDG_DATA_HOME");
    if (xdgDataHome != NULL) {
        basePath = std::string(xdgDataHome) + "/" + appName;
    } else {
        basePath = std::string(homeDir) + "/.local/share/" + appName;
        
        // Создаем директории, если они не существуют
        std::string localDir = std::string(homeDir) + "/.local";
        std::string shareDir = localDir + "/share";
        
        if (!directoryExists(localDir)) {
            createDirectory(localDir);
        }
        
        if (!directoryExists(shareDir)) {
            createDirectory(shareDir);
        }
    }
    
    // Создаем директорию приложения, если она не существует
